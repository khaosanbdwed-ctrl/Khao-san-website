...
export default function AdminMenu() {
    ...
    return (
        <div>
            ...
            {/* Menu Data Table */}
            <div style={{ backgroundColor: 'var(--color-surface-base)', borderRadius: '12px', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr key={item.id} style={{ borderTop: '1px solid var(--color-border)' }}>
                                ...
                                <td style={{ padding: '20px 24px' }}>
                                    <span style={{ padding: '4px 12px', backgroundColor: 'var(--color-border)', borderRadius: '16px', fontSize: '0.85rem' }}>{item.category}</span>
                                </td>
            ...
            {/* Add Modal */}
            {isModalOpen && (
                <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(22,22,22,0.08)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                    <div style={{ backgroundColor: 'var(--color-surface-base)', padding: '40px', borderRadius: '12px', width: '100%', maxWidth: '500px', border: '1px solid rgba(22,22,22,0.12)' }}>
                        ...
                        <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} style={{ width: '100%', padding: '12px', backgroundColor: 'rgba(22,22,22,0.05)', border: '1px solid rgba(22,22,22,0.12)', borderRadius: '8px', color: 'var(--color-text-primary)' }} />
                            </div>

                            <div style={{ display: 'flex', gap: '20px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
                                    <label style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Category</label>
                                    <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} style={{ width: '100%', padding: '12px', backgroundColor: 'rgba(22,22,22,0.05)', border: '1px solid rgba(22,22,22,0.12)', borderRadius: '8px', color: 'var(--color-text-primary)' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
                                    ...
                                    <input type="text" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} style={{ width: '100%', padding: '12px', backgroundColor: 'rgba(22,22,22,0.05)', border: '1px solid rgba(22,22,22,0.12)', borderRadius: '8px', color: 'var(--color-text-primary)' }} />
                                </div>
                            </div>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                <label style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Description</label>
                                <textarea required rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} style={{ width: '100%', padding: '12px', backgroundColor: 'rgba(22,22,22,0.05)', border: '1px solid rgba(22,22,22,0.12)', borderRadius: '8px', color: 'var(--color-text-primary)', resize: 'vertical' }} />
                            </div>

                            {/* Pseudo Image Upload */}
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                <label style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Photo Upload (PNG only)</label>
                                <div style={{ width: '100%', padding: '24px', backgroundColor: 'rgba(22,22,22,0.05)', border: '1px dashed rgba(22,22,22,0.12)', borderRadius: '8px', textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>
                                    Click to upload a PNG file. (Supabase Storage placeholder)
                                </div>
                            </div>

                            <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
                                <button type="button" onClick={() => setIsModalOpen(false)} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid rgba(22,22,22,0.12)', color: 'var(--color-text-primary)', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
